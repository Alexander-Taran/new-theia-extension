...
test.serial('extention files are placed in a folder with name of the extension', async t => {
...
test.serial('extention files are placed in a nested folder with name of the extension', async t => {


  const jsExt = extTransform({ name: '@scope/my-app' },['standalone']);
  ...
  const files = await jsExt.toArray()
  t.is(files.length, 2);
  t.is(files[0].path.replace(/\\/g, '/'), 'package.json');
  t.is(files[0].contents.toString(), '{}');
  t.is(files[1].path.replace(/\\/g, '/'), 'my-app-module.ts');
  t.is(files[1].contents.toString(), '{}');
